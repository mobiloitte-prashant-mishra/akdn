content_migrate_unt_ is responsible for migrating data from Drupal 6 to Drupal 7.

content_migrate_unt_ supports migrating content from a Drupal 6 to Drupal 7 using the Migrate module. Import of the following
is supported:
...
IMPORTANT: Set the source drupal database name and table prefix at <your_site_url>?q=admin/settings/spandan5
Use "mckc_" as table prefix, since out Drupal5 db is using this prefix.
NOTE: If your current drupal 6 instance is using any prefix, use this prefix in the source database naming and in the setting only pass the database name excluding the prefix.
e.g. spandan6 is using prefix "spandan_"
so the source database name must be "spandan_your_dbname"
at <your_site_url>?q=admin/settings/spandan5 path only pas "your_dbname"


FOR Article:
1. For story migration run the command:
    drush mi SpandanStoryNode
   For rollback of the migration: 
    drush mr SpandanStoryNode

	For comments migration
	drush mi SpandanStoryComment
